...
// AnalyseGenomeMFCDlg.cpp : fichier d'implÈmentation
//
...
BOOL CAnalyseGenomeMFCDlg::OnInitDialog()
{
	...
	// Ajouter l'ÈlÈment de menu "¿ propos de..." au menu SystËme.

	// IDM_ABOUTBOX doit se trouver dans la plage des commandes systËme.
	...
	// TODO: ajoutez ici une initialisation supplÈmentaire

	return TRUE;  // retourne TRUE, sauf si vous avez dÈfini le focus sur un contrÙle
}
...
void CAnalyseGenomeMFCDlg::OnPaint()
{
	if (IsIconic())
	{
		CPaintDC dc(this); // contexte de pÈriphÈrique pour la peinture

		SendMessage(WM_ICONERASEBKGND, reinterpret_cast<WPARAM>(dc.GetSafeHdc()), 0);

		// Centrer l'icÙne dans le rectangle client
...
void CAnalyseGenomeMFCDlg::OnBnClickedButton1()
{
	...
	if (nomCommande.compare("connexionMedecin") == 0)
	{
		...
		if (connexion == 0)
		{
			CString messageC(">> Connexion effectuÈe !\r\n");
		...
		{
			CString messageNC(">> Connexion ÈchouÈe. Essayez encore !\r\n");
	...
	if (medecinConnected != -1)
	{
		if(nomCommande.compare("deconnexionMedecin") == 0) {
			...
			if (deconnexion)
			{
				CString messageD(">> DÈconnexion effectuÈe !\r\n");
			...
			{
				CString messageND(">> La dÈconnexion a ÈchouÈe !\r\n");
			...
			if (infos.size() != 4)
			{
				CString messageE(">> ProblËme lors de la crÈation d'un patient au niveau des arguments fournis\r\n");
			...
			{
				CString messageDPE(">> Erreur lors de la crÈation du patient !\r\n");
			...
			CString messageR1(">> Voici le rÈsultat de cette analyse :\r\n");
